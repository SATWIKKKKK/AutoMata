...
export default function Workflows({ onViewWorkflow }: WorkflowsProps) {
  ...
  return (
    <div className="p-8 md:p-12 max-w-[1440px] mx-auto w-full space-y-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-blueprint-line pb-8 gap-4">
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        {workflows.map((workflow, idx) => (
          <motion.div
            ...
            className="group flex flex-col md:flex-row md:items-center justify-between border border-blueprint-line bg-surface-container-lowest p-6 rounded-2xl hover:border-blueprint-accent transition-all shadow-sm hover:shadow-md gap-6"
          >
            <div className="flex items-center gap-6 flex-1">
              <div className={cn(
                "w-12 h-12 rounded-xl flex items-center justify-center shrink-0 border border-blueprint-line/50",
                workflow.status === 'active' ? "bg-green-50 text-green-600" : "bg-blueprint-bg text-blueprint-muted"
              )}>
                <Shield size={24} />
              </div>
              <div className="min-w-0">
                <div className="flex items-center gap-3 mb-1">
                  <h3 className="text-ui-label text-blueprint-accent text-lg leading-none">{workflow.name}</h3>
                  <span className={cn(
      ...
      <div className="border border-dashed border-blueprint-line rounded-2xl p-12 flex flex-col items-center justify-center text-center bg-blueprint-bg/20">
         ...
         <div className="flex gap-12">
           ...
           <div className="text-center border-x border-blueprint-line px-12">
